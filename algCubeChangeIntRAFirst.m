    ...
    while ~terminate
        ...
        if size(missPts,2)>0 && size(addPts,2)>0
            ...
            while ~done
                ...
                TPidx=TPidx+1;
                % Bug TravelPaths{TPidx}= horzcat(tgtCubeID, srcPt, derivedCloudPoint.vertexList( srcPt ), cid, destPt, leadCloudPoint.vertexList( destPt ) );
                TravelPaths{TPidx}= horzcat(tgtCubeID, srcPt, leadCloudPoint.vertexList( srcPt ), cid, destPt, derivedCloudPoint.vertexList( destPt ) );
